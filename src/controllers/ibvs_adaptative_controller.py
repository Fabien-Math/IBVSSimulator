...
class IBVSADAPController:
	def __init__(self, controller_params, mission_params, camera, world):
		...
		### IBVS parameters
		self.lambda_min = controller_params['params']['lambda_min']
		self.lambda_max = controller_params['params']['lambda_max']
		func = controller_params['params']['func']
		if func == 'linear':
			self.interp_lambda = self.interp_lambda_linear
		if func == 'quad':
			self.interp_lambda = self.interp_lambda_quad
	...
	def update(self, eta, nu, dt, time):
		...
		lambda_gain = self.interp_lambda()

		ibvs_cmd = lambda_gain * self.camera.Vrc @ (L_e_p @ self.errors)
		centering_cmd = lambda_gain * self.camera.Vrc @ (L_center_p @ cog_error)

		self.cmd = (1 - ratio) * ibvs_cmd + ratio * centering_cmd


	def interp_lambda_linear(self):
		lambda_gain = (self.lambda_max - self.lambda_min) * self.error_norm + self.lambda_min
		return np.where(lambda_gain > self.lambda_max, self.lambda_max, lambda_gain)

	def interp_lambda_quad(self):
		lambda_gain = (self.lambda_max - self.lambda_min) * self.error_norm**2 + self.lambda_min
		return np.where(lambda_gain > self.lambda_max, self.lambda_max, lambda_gain)
