...
class IBVSPIDController:
	def __init__(self, controller_params, mission_params, camera, world, **kwargs):
		self.camera = camera
		self.img = None
		self.new_img = False
		self.xs = None
		...
		### IBVS parameters
		self.kp = controller_params['params']['kp']
		self.ki = controller_params['params']['ki']
		self.inte_err = np.zeros(6)
		self.last_err = np.zeros(6)
		self.kd = controller_params['params']['kd']
	...
	def update(self, eta, nu, dt, time):
		...
		ibvs_err = self.camera.Vrc @ (L_e_p @ self.errors)
		centering_err = self.camera.Vrc @ (L_center_p @ cog_error)
		err = (1 - ratio) * ibvs_err + ratio * centering_err

		self.inte_err += err * dt
		self.inte_err = np.clip(self.inte_err, -10, 10)

		der_err = 0
		if dt:
			der_err = (err - self.last_err)/dt
		self.last_err = err

		print(self.error_norm, self.inte_err)

		self.cmd = self.kp * err + self.ki * self.inte_err + self.kd * der_err
