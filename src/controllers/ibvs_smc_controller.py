...
class IBVSSMController:
	def __init__(self, controller_params, mission_params, camera, world, **kwargs):
		...
		self.k = controller_params['params']['k']
		self.phi = controller_params['params']['phi']
	...
	def update(self, eta, nu, dt, time):
		...
		self.s = self.lambda_gain * err - nu
		self.u = self.k * np.array([np.tanh(si / self.phi[i]) for i, si in enumerate(self.s)])
		self.cmd = self.u
