...
class Renderer:
	def __init__(self, timestep, graphical=True, window_width=1600, window_height=900):
		# Viewport variables
		...
		self.dt = timestep
		self.time = 0.0
		self.interframe_delay = 40 # millisecond
	...
	def render_onboard_camera(self):
		if self.img is not None:
			if self.time <= self.last_img_ts + 1 / self.onboard_camera.fps:
				return self.img
	...
	def update(self, value):
		...
		# If mission finished, stop updating the simulation but continue rendering window
	...
	def run(self, robot, world, ocean):
		...
		if self.graphical:
			...
			self.interframe_delay = 1 # ms (Nearly no delay between frame because no window is rendered)
